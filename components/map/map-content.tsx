  ...
  return (
    <div className="relative flex h-[calc(100vh-64px)] w-full overflow-hidden bg-background">
            ...
            <div className="space-y-4">
              ...
              {displayData.length === 0 ? (
                ...
                displayData.map((item) => {
                    ...
                    return (
                        <Card key={item.id} className={`overflow-hidden group hover:shadow-md transition-all duration-200 border-l-4 ${isLowPriority ? 'opacity-60 grayscale-[0.5] hover:opacity-100 hover:grayscale-0' : ''}`}
                        ...
                        >
                            ...
                            <div className="relative h-40 w-full bg-slate-100">
                                ...
                                <div className="absolute top-2 right-2">
                                    <Badge className={severity === 'High' || severity === 'Critical' ? 'bg-red-500 hover:bg-red-600' : severity === 'Medium' ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-green-500 hover:bg-green-600'}>
                                        {severity} Risk
                                    </Badge>
                                </div>
                            </div>
                            ...
                            <CardContent className="p-4">
                                ...
                                {/* --- AUDIT VERDICT --- */}
                                <div className={`mb-3 p-2.5 rounded border text-xs ${isPrediction ? 'bg-purple-50 border-purple-100' : 'bg-slate-50 border-slate-100'}`}>
                                    <div className="font-semibold text-slate-700 mb-1 flex items-center gap-2">
                                        {isPrediction ? <BrainCircuit className="h-3.5 w-3.5 text-purple-600" /> : <ShieldCheck className="h-3.5 w-3.5" />}
                                        <span>{isPrediction ? "AI Prediction:" : "Integrity Audit:"}</span>
                                        {!isPrediction && (
                                            isMatched ? (
                                                severity === 'High' ? <span className="text-red-600 font-bold">⚠️ Mismatch</span> : <span className="text-green-600 font-bold">✅ Verified</span>
                                ...
                                {/* --- ACTION BAR --- */}
                                {isLowPriority && (
                                    <div className="mb-3 text-[10px] text-orange-600 bg-orange-50 px-2 py-1 rounded flex items-center gap-1">
      ...
      <div className="flex-1 relative h-full w-full z-0">
        <LeafletMap 
          reports={reports} // Pass FULL LIST to Leaflet so it can calculate bounds
